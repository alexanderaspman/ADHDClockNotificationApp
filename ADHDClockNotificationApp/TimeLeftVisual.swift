...
var body: some View {
    ...
    var rectangleTimer: some View {
        ...
        let progress = 1 - ( percent/130)
        
        
        return Rectangle().trim(from: progress, to: 1)            .stroke(LinearGradient(gradient: Gradient(colors: [Color(color1), Color(color2)
    ...
    var circleTimer: some View{
        ...
        VStack{
            ...
            Circle().trim(from: progress, to: 1).stroke(Color(color3),style: StrokeStyle(lineWidth: 5*multiplier, lineCap: .round,lineJoin: .round, miterLimit: .infinity,dash: [0,20],dashPhase: 0)).rotationEffect(Angle(degrees: 274)).rotation3DEffect(Angle(degrees: 0), axis: (x:4,y:2,z:0))            .frame(height: 200).position(x:200,y: 600)
            
            }
         
                
            Text("\(percent) minutes left").foregroundColor(.white).font(.title).bold().frame(width:  160,height: 80 ).position(x:200,y: 600).shadow(color:Color(color5[1]).opacity(0.91934),radius: 30)                 }
    }
